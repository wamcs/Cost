...
public class BillWrite extends BaseActivity {
    ...
    public void initDate(int billitemID){
        if (billitemID<=0){
            ...
            recycleperiod=Util.NOPERIOD;
            getLabel.setText(billDataHelper.getLabel(1));
    ...
    public void dateCofirm(){
        ...
        map.put("label",label);
        map.put("color", billDataHelper.getColor(label));
        map.put("period",recycleperiod);
        map.put("year",year);
        map.put("month",month);
        map.put("date",date);
        map.put("week",TimeHelper.getWeek());
        map.put("time",times);
        ...
        if(billitemID<=0) {
            ...
            if((int)billmap.get("period")==Util.NOPERIOD)
            {
                ...
                for(int i=0;i<list.size();i++)
                    billDataHelper.updateBillRecycle(map,list.get(i));
                if (periodswitch.isChecked())
                    billDataHelper.updateRecycle(map, recycleid);
                else
    ...
    public void popLabel(){
        ...
        LabelChooseAdapter adapter=new LabelChooseAdapter(BillWrite.this, billDataHelper.getLabelColor());
